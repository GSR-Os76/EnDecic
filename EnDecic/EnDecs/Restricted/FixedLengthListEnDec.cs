...
namespace GSR.EnDecic.Implementations.Restricted
{
    public sealed class FixedLengthListEnDec<T> : IEnDec<IList<T>>
    {
        ...
        {
            IList<T> data = codingSet.IsNotNull().DecodeList(stream.IsNotNull(), _enDec);
        ...
        public U Encode<U>(IEncodingSet<U> codingSet, IList<T> data) => data.Count == _length ? codingSet.IsNotNull().EncodeList(data.IsNotNull(), _enDec) : throw new ArgumentException($"Expected list of the length: {_length}, got {data.Count} instead.");
