...
namespace GSR.EnDecic.ByteStream
{
    public sealed class ByteStreamCodingSet : ICodingSet<IQueue<byte>>
    {
        ...
        /// <inheritdoc/>
        public IDictionary<K, V> DecodeMap<K, V>(IQueue<byte> stream, IDecoder<K> keyTypeDecoder, IDecoder<V> valueTypeDecoder) => new ImmutableOrderedDictionary<K, V>(DecodeInt32(stream).XTimes<object>().Select((x) => KeyValuePair.Create(keyTypeDecoder.Decode(this, stream), valueTypeDecoder.Decode(this, stream))).ToArray());
        ...
        /// <inheritdoc/>
        public IQueue<byte> EncodeMap<K, V>(IDictionary<K, V> data, IEncoder<K> keyEncoder, IEncoder<V> valueEncoder) => data.Aggregate(EncodeInt32(data.Count), (seed, x) => seed.Enqueue(keyEncoder.Encode(this, x.Key).DequeueAll()).Enqueue(valueEncoder.Encode(this, x.Value).DequeueAll()));
